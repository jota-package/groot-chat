...
const tmp_path = function(path_route){
    tmp_dir = path_route;
}
...
const img2tiff = (filepath, callback) => {
    ...
    Jimp.read(filepath)
        .then(image => {
            ...
            let ruta = path.join(tmp_dir, filename);
...
const tiff2img = function (filepath, callback){
    ...
    tiff2ArrayBuffer(filepath, async function(pages, data) {
        ...
        let ruta_out = path.join(tmp_dir, output_dir);
        ...
        for(let i in pages){
            ...
            if(page_data.rgba.length != 0){
                let ruta_out_file = path.join(tmp_dir, output_dir+'/'+original_file+'_'+i+'.jpg');
...
const PDF2tiff = function (filepath, callback){
    ...
    https.get(filepath, function(response) {
        let filename = nameGenerator(10)+extension;
        let ruta_out = path.join(tmp_dir, filename);
            ...
            if(num_pages_pdf >= first_page){
                ...
                https.get(filepath, function(response) {
                    ...
                    for(let i=(first_page-1); i<last_page; i++){
                        let ruta_out = path.join(tmp_dir, output_dir+'/'+output_name+'_'+i+'.jpg');
